...
class LoginForm(forms.Form):
    # widget可以设置字段的类型，比如密码为密文类型
    username_or_email = forms.CharField(
        label="用户名或邮箱",
        widget=forms.TextInput(attrs={'class':'form-control','placeholder':'请输入用户名或邮箱'})) # required表示用户名不能为空需要有输入，自动监测并有提示
    password = forms.CharField(label="密码", widget=forms.PasswordInput(attrs={'class':'form-control','placeholder':'请输入密码'})) # 变成password密文的类型
    ...
    def clean(self):
        ...
        if not user:
            ...
            self.cleaned_data['user'] = user # 用户名密码验证成功将user传给视图函数进行登录
        return self.cleaned_data
...
class RegForm(forms.Form):
    ...
    def __init__(self,*args,**kwargs):
        if 'request' in kwargs: # 获取user关键字参数
